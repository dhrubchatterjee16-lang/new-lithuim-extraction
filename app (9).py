...
with tabs[0]:
    ...
    with col1:
        ...
        fig_gauge = go.Figure(go.Indicator(
            ...
            number={
                'suffix': '%',
                'font': {'size': 52, 'color': '#f1f5f9', 'family': 'Arial Black'}
            },
            title={
                'text': f"Li Recovery vs Benchmark ({bench_li}%)",
                'font': {'size': 13, 'color': '#94a3b8'}
            },
            gauge={
                'axis': {'range': [0, 100], 'tickwidth': 1,
                         'tickcolor': '#64748b', 'tickfont': {'size': 11}},
    ...
    with col2:
        ...
        tip_html = "".join(f"<p style='margin-bottom:10px;color:#cbd5e1;font-size:13px'>{t}</p>" for t in tips)
...
with tabs[1]:
    ...
    with col2:
        ...
        for item in interp:
            st.markdown(f'<div class="tip-box" style="margin-bottom:6px;color:#cbd5e1">{item}</div>', unsafe_allow_html=True)
...
with tabs[2]:
    ...
    for idx, (target, fvals) in enumerate(fi.items()):
        ...
        with col:
            r2 = MODEL_RESULTS[target].get('r2_blend', MODEL_RESULTS[target].get('r2_xgb', 0))
